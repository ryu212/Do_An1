...
QueueHandle_t sd_card_data_queue;
...
void spi_slave_task(void *arg)
{
    ...
    while (1) {
        ...
        if (ret == ESP_OK) {
            ...
            printf("\n");
            xQueueSend(mqtt_data_queue, data, portMAX_DELAY);
            vTaskDelay(pdMS_TO_TICKS(100));
            xQueueSend(sd_card_data_queue, data, portMAX_DELAY);
...
void init_spi_queue(void) {
    ...
    mqtt_data_queue = xQueueCreate(5, sizeof(float) * ELEMENTS);
    sd_card_data_queue = xQueueCreate(15, sizeof(float) * ELEMENTS);
